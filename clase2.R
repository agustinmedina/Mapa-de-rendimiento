...
# 1. CARGAR DATOS
puntos_raw <- vect("mapa_rinde.shp") 
...
# ---------------------------------------------------------
# NUEVO: BOXPLOT COMPARATIVO (ANTES Y DESPUÉS)
# ---------------------------------------------------------
# Configuramos la pantalla para ver los dos gráficos juntos
...
# Volvemos a la configuración de 1 solo gráfico por pantalla
par(mfrow = c(1, 1))
# ---------------------------------------------------------

# 4. CONVERSIÓN A SF Y PROYECCIÓN. Aca transformo a metros.
...
kriging_res <- krige(yield ~ 1, puntos_sf, grid, model = v_fit)
# Resetear la pantalla a 1 solo gráfico (1 fila, 1 columna)
dev.off() 
...
writeRaster(rinde_final_wgs84, "rinde_final_ok.tif", overwrite = TRUE)
#Hasta acá a partir de MR > limpieza > raster para ambientar en base a datos de rinde

#AMBIENTACION A PARTIR DE MR#
# ============================================================
# AMBIENTACIÓN: RASTER DE RINDE → POLÍGONOS CONTIGUOS SIN HUECOS
# ============================================================

# --- Paso 1: Definir número de ambientes ---
n_ambientes <- 5  # Podés cambiar a 2, 4, 5, etc.
...
# Crear la matriz de reclasificación
rcl_mat <- cbind(
...
# Filtro focal de moda (ventana 5x5, ajustá según resolución)
rinde_suave <- focal(rinde_class, w = 5, fun = "modal", na.rm = TRUE)

# Reaplicar la máscara del lote
...
# En vez de iterar uno a uno, rasterizar de vuelta y re-poligonizar
# Esto es mucho más rápido que el loop con st_touches

# Convertir a raster, aplicar focal más agresivo y volver a poligonizar
...
# Generar etiquetas dinámicas según n_ambientes
if (n_ambientes == 2) {
...
# Cerrar todo dispositivo gráfico colgado
graphics.off()

# Simplificar fuerte para que no se trabe
ambientes_plot <- st_simplify(ambientes_sf, dTolerance = 0.0002)
ambientes_plot <- st_make_valid(ambientes_plot)

# Graficar la versión liviana
...
# Paleta dinámica de rojo a verde con N colores
colores_amb <- colorRampPalette(c("red", "gold", "darkgreen"))(n_ambientes)
...
st_write(ambientes_sf, "ambientes_rinde.shp", delete_dsn = TRUE)

cat("\n✔ Shapefile exportado: ambientes_rinde.shp\n")
...
#ACÁ SIGUE EL CODIGO ORIGINAL LIMPIEZA + ANALISIS

# 8. ANÁLISIS BIVARIADO NDVI
s2_img <- rast("Sentinel2_RGB_2022_01.tif")
ndvi_raw <- (s2_img[["B8"]] - s2_img[["B4"]]) / (s2_img[["B8"]] + s2_img[["B4"]])
