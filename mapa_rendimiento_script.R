# install.packages(c("terra", "sf", "gstat", "dplyr", "lwgeom"))
library(terra)
library(sf)
library(gstat)
library(dplyr)
...
# 1. CARGAR DATOS
# mapa_rinde.shp mezcla puntos y polígonos; vect() solo carga uno de los dos.
capas <- svc("mapa_rinde.shp")
idx_puntos <- which(sapply(capas, function(capa) geomtype(capa) == "points"))
if (length(idx_puntos) == 0) {
  stop("No se encontró capa de puntos en mapa_rinde.shp")
}
puntos_raw <- capas[idx_puntos[1]]
lote_raw   <- vect("lote.shp")

# 2. EXTRACCIÓN DE COORDENADAS Y RINDE
attr_df <- as.data.frame(puntos_raw, geom = "XY")
df_base <- data.frame(
  x = attr_df$x,
  y = attr_df$y,
  yield = as.numeric(attr_df$VRYIELDMAS)
)
